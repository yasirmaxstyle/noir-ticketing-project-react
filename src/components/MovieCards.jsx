import { Link } from "react-router"

export default function MovieCards({ category, title, genres, release, src, id }) {
  return (
    <>
      <div className="flex grow flex-col gap-3 items-center shadow-xl rounded-xl shrink-0 basis-[calc((1/4*100%)-12px)] bg-graphite group" key={`${category}-${id}`}>
        <div className="w-full overflow-hidden rounded-xl relative">
          <div className="hidden bg-black opacity-50 absolute w-full h-full z-[1] group-hover:block" />
          <div className="hidden absolute inset-0 translate-y-1/3 group-hover:grid justify-center z-[2]">
            <div className="flex flex-col gap-3">
              <Link to={`/movie/${id}`} state={id} className="border border-white px-5 py-3 rounded">
                <span className="text-white">
                  VIEW DETAILS
                </span>
              </Link>
              <Link className="bg-black px-5 py-3 rounded">
                <span className="text-white">
                  BUY TICKET
                </span>
              </Link>
            </div>
          </div>
          <img src={src} alt={title} className="group-hover:scale-110 group-hover:rotate-1 ease-in duration-300 grayscale group-hover:grayscale-0" />
        </div>
        <div className="p-3 w-full flex flex-col items-center gap-3 text-center">
          <h4 className="font-bold text-platinum">{title}</h4>
          <div className="flex gap-2 flex-wrap py-3">
            {genres?.map(genre => {
              return (
                <span key={`${category}${genre}`} className="text-graphite bg-ash rounded-full px-3 py-0.5">{genre}</span>
              )
            })
              || <span className="text-ash">{release}</span>}
          </div>
        </div>
      </div>
    </>
  )
}